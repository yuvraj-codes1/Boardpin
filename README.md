...
- 🤖 **AI-generated short labels** — every shared or pasted link gets a clean, auto-generated label via a serverless function backed by Google's Gemini API; YouTube links even get their thumbnail analyzed for a sharper label
...
api/
  label.js            # Vercel serverless function — AI title generation (keeps API key server-side)
...
### Setting up AI labeling (Gemini API)

The AI label feature runs through a small serverless function (`api/label.js`) so your API key is never exposed in the browser.

1. Get a free API key at [aistudio.google.com](https://aistudio.google.com/apikey) (Google AI Studio — no credit card required)
2. In your Vercel project → **Settings → Environment Variables**, add:
   - Name: `GEMINI_API_KEY`
   - Value: your Google AI Studio key
   - Environment: Production (and Preview if you use it)
3. Redeploy — Vercel automatically detects `api/label.js` as a serverless function, no extra config needed
4. If the key is missing or invalid, the app silently falls back to the non-AI caption cleanup instead of breaking — check your Vercel function logs if labels aren't appearing

**Model used:** `gemini-2.5-flash-lite` — Google's fastest, cheapest current Gemini model, still free at this usage volume (personal bookmark-labeling), and the config disables "thinking" tokens and caps output length so each call stays minimal.

**What's genuinely smarter than before:** for YouTube links, the function fetches the video's public thumbnail and sends it to Gemini as an actual image, not just text — so labels can reflect what's visually in the video, not just the title.

**Honest limitation:** Instagram, TikTok, and most other platforms block fetching their post images from a server without paid scraping tools, so those labels are still generated from text only (URL + whatever caption Android includes in the share) — the AI is not "seeing" the Instagram photo/video itself.
